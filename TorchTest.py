...
@tictoc
def trainmodel(data, data2, learning_rate, num_epochs, num_neurons, rel_tol, abs_tol, live_plot):
    """
    ...
    for epoch in range(num_epochs):
        ...
        if isinstance(optimizer, torch.optim.LBFGS):
            ...
            optimizer.zero_grad()
            pred_y = odeint(net, features[0], t, rtol=rel_tol, atol=abs_tol, method="dopri8")
            loss = loss_function(pred_y, features)
            loss.backward()
            optimizer.step()
...
network, losses = trainmodel(train_data, val_data, 0.01, 50, 50, 1e-7, 1e-5, False)
